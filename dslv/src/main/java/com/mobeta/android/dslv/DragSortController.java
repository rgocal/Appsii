...
public class DragSortController extends SimpleFloatViewManager
    ...
    static final int REMOVE_MODE_WAITING = 0;

    static final int REMOVE_MODE_REMOVING = 1;

    static final int REMOVE_MODE_FAKE_REMOVING = 2;

    int mIsRemoving = REMOVE_MODE_WAITING;

    private float mMaxFakeRemoveDistance;
    ...
    public DragSortController(DragSortListView dslv, int dragHandleId, int dragInitMode,
        ...
        super(dslv);
        mDslv = dslv;
        Context context = dslv.getContext();
        mMaxFakeRemoveDistance = 16 * context.getResources().getDisplayMetrics().density;
        mDetector = new GestureDetector(context, this);
        mFlingRemoveDetector = new GestureDetector(context, mFlingRemoveListener);
        mFlingRemoveDetector.setIsLongpressEnabled(false);
        mTouchSlop = ViewConfiguration.get(context).getScaledTouchSlop();
        mDragHandleId = dragHandleId;
        mClickRemoveId = clickRemoveId;
        mFlingHandleId = flingHandleId;
        mFlingSpeed = 1000 * context.getResources().getDisplayMetrics().density;
    ...
    public boolean startDrag(int position, int deltaX, int deltaY) {
        ...
        int dragFlags = 0;
        if (mSortEnabled && mIsRemoving == REMOVE_MODE_WAITING) {
            dragFlags |= DragSortListView.DRAG_POS_Y | DragSortListView.DRAG_NEG_Y;
        }
        if (mRemoveEnabled && mIsRemoving != REMOVE_MODE_WAITING) {
    ...
    public boolean onTouch(View v, MotionEvent ev) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_UP:
                if (mRemoveEnabled && mIsRemoving != REMOVE_MODE_WAITING) {
                    int x = Math.abs(mPositionX);
                    int removePoint = mDslv.getWidth() / 2;

                    if (x > removePoint && mIsRemoving == REMOVE_MODE_REMOVING) {
                        mDslv.stopDragWithVelocity(true, 0);
                    }
                }
            case MotionEvent.ACTION_CANCEL:
                mIsRemoving = REMOVE_MODE_WAITING;
                mDragging = false;
                break;
        }
    ...
    public void onDragFloatView(View floatView, Point position, Point touch) {
        if (mRemoveEnabled && mIsRemoving != REMOVE_MODE_WAITING) {
            ...
            if (mIsRemoving == REMOVE_MODE_FAKE_REMOVING) {
            ...
            if (mHitPos != MISS) {
                if (mDragInitMode == ON_DRAG && Math.abs(y2 - y1) > mTouchSlop && mSortEnabled) {
                    startDrag(mHitPos, deltaX, deltaY);
                        ...
                        mRemoveEnabled) {
                    if (canRemove(mFlingHitPos)) {
                        mIsRemoving = REMOVE_MODE_REMOVING;
                    } else {
                        mIsRemoving = REMOVE_MODE_FAKE_REMOVING;
                    }
    ...
    boolean canRemove(int flingHitPos) {
        ListAdapter adapter = mDslv.getInputAdapter();
        if (adapter instanceof ConditionalRemovableAdapter) {
            ConditionalRemovableAdapter conditionalRemovableAdapter =
                    (ConditionalRemovableAdapter) adapter;
            return conditionalRemovableAdapter.canRemove(flingHitPos);
        }
        return true;
    }
    ...
    private GestureDetector.OnGestureListener mFlingRemoveListener =
            new GestureDetector.SimpleOnGestureListener() {
                ...
                public final boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX,
                    ...
                    if (mRemoveEnabled && (mIsRemoving == REMOVE_MODE_REMOVING)) {
